...
func Init(c Config){
	...
	zero.Default().OnRegex("^电阻(.*?)$").SetBlock(true).SetPriority(20).Handle(func(ctx *ZeroBot.Ctx) {
	...
	zero.Default().OnRegex("^尺寸(.*?)$").SetBlock(true).SetPriority(20).Handle(func(ctx *ZeroBot.Ctx) {
		code := ctx.State["regex_matched"].([]string)[1]
