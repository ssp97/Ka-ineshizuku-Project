...
func Init(c Config) { // 插件主体
	...
	for i := range POOL.List {
		// 下载全部图
		//num, _ := POOL.DB.Num(POOL.List[i])
		//for j := 0; j < num; j++ {
		//	illust := &pixiv.Illust{}
		//	POOL.DB.Select(POOL.List[i], illust, fmt.Sprintf("limit 1 offset %d", j))
		//	go download(illust, POOL.Path)
		//	fmt.Printf("正在下载%s第%d张\n",POOL.List[i],j+1)
		//}
		go func(label string) {
			for j := 0;j <5;j++{
				illust := &pixiv.Illust{}
				fmt.Printf("[缓存图片]正在下载%s第%d张\n",label,j+1)
				POOL.DB.Select(label, illust, "ORDER BY RANDOM() limit 1")
				if _, err := download(illust, POOL.Path); err!= nil{
					fmt.Printf("[缓存图片]正在下载%s第%d张失败\n",label,j+1)
					continue
				}
				POOL.Push(label, illust)
			}
		}(POOL.List[i])
		...
		Handle(func(ctx *ZeroBot.Ctx) {
			...
			go func() {
				...
				for i := 0; i < times; i++ {
					...
					if _, err := download(illust, POOL.Path); err != nil {
						//ctx.SendChain(message.Text("ERROR: ", err))
						//ctx.SendChain(message.Text("我发了，但图好像被马爸爸吃了"))
						continue
					}
			...
			if POOL.Size(type_) == 0 {
				ctx.SendChain(message.Text("找图去了，稍安勿躁"))
